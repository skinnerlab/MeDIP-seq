...
for (analysis in 1:length(comparison)) {  
  ...
  for (pV in 1:length(methList)) {
    if (!is.null(methList[[pV]])) {
      oneUse <- strsplit(as.character(rbind(methListEtc[[pV]])[, "edgeR.p.value"]), split = ";")
  ...
  for (pV in 1:length(MTCmethList)) {
    if (!is.null(MTCmethList[[pV]])) {
      oneUse <- strsplit(as.character(rbind(MTCmethListEtc[[pV]])[, "edgeR.p.value"]), split = ";")
  ...
  # create table of DMR numbers and save to file in results. This will be used to select the 
  # p-value to use for further analysis.
