...
## Note: (by Daniel Beck)
## Haque developed this function to identify clusters of DMR. It counts DMR that fall within
## a sliding window across the genome. It then determines which windows have significantly
## elevated levels of DMR using a normal distribution parameterized with the total number of
## DMR and the total sequence length. The significance threshold is hardcoded at p < 0.05. 
## This seems to be a terrible way of doing things and probably results in unreasonably high 
## confidence in the identified clusters. I've left the code alone, only changing formating 
## for consistency. This function should be replaced with a more appropriate cluster detection 
## method. Haque's original comments are kept below.

# Input parameters 
#   (1) table containing field Choromosome, cSTART, cSTOP, minP and nProbes
...
#	  i.e. cluster_table <- Clustering(ctable, 2000000, 50000)
# Output
...
slidingWindowCluster <- function(ctable, windowLength, incrementLength) {
  ...
  cmatrix <- matrix(0, nrow = 100000, ncol = 6, 
                    dimnames = list(c(1:100000), c("Chromosome", "cSTART", "cSTOP",
    ...
    if (nrow(ctable[[k]]) == 0) {
      ...
      for (i in 1:limit) {
        ...
        if (count >= 1) {
          ...
          seq <- paste("chromosome :", char, "start: ", as.integer(i), " end: ", 
                       as.integer(m), "count: ", as.integer(count))
          #		print(seq)
  ...
  clusters <- data.frame(
    Chromosome = as.character(sliding_window[, "Chromosome"]),
      ...
      for (j in 1:limit) {
        ...
        if ((end2 - end1) < 1000000) {
          if (j == limit) {
            cluster_end <- 	as.numeric(clusters[[i]][j,"cSTOP"])
            Seq <- data.frame(
              Chromosome = as.character(name), cSTART = as.numeric(cluster_start), 
              cSTOP = as.numeric(cluster_end), minP = as.numeric(cluster_p_value))
            #	print(Seq)
            cmatrix <- rbind(cmatrix, Seq)
          }
        }
