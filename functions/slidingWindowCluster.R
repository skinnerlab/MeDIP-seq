...
slidingWindowCluster <-
  function(ctable, windowLength, incrementLength)
  {
    if (is.null(nrow(ctable)))
      return(NULL)
    if (nrow(ctable) <= 1)
      return(NULL)
    
    ctable <-
      ctable[c("Chromosome","cSTART","cSTOP","minP","nProbes")]
    ctable <- split(ctable, ctable["Chromosome"])
    
    cmatrix <-
      matrix(0, nrow = 100000, ncol = 6, dimnames = list(
        c(1:100000),c(
          "Chromosome", "cSTART","cSTOP","Count", "Z-score","P-value"
        )
      ))
    index <- 0
    ...
    for (k in 1:length(names(ctable)))
    {
      if (nrow(ctable[[k]]) == 0)
      {
        ...
        count <- 0
        limit <-
          as.integer (max(ctable[[k]][,"cSTOP"]) / incrementLength)
        ...
        for (i in 1:limit)
        {
          ...
          if (count >= 1)
          {
            ...
            seq <-
              paste(
                "chromosome :", char , "start: ",as.integer(i), " end: ", as.integer(m), "count: ", as.integer(count)
              )
            #		print(seq)
    ...
    sliding_window <-
      sliding_window[1:(head(which(cmatrix[,"Chromosome"] == 0),1) - 1),]
    sliding_window[,"cSTART"] <-
      as.numeric(as.character(sliding_window[,"cSTART"]))
    sliding_window[,"cSTOP"] <-
      as.numeric(as.character(sliding_window[,"cSTOP"]))
    sliding_window[,"Count"] <-
      as.numeric(as.character(sliding_window[,"Count"]))
    ...
    Z <-
      apply(as.matrix(X,ncol = 1), 2, function(x)
        (x - meanM) / sdM)
    ...
    sliding_window <-
      sliding_window[order(sliding_window[,"Chromosome"],sliding_window[,"cSTART"]),]
    sliding_window <-
      sliding_window[which(as.numeric(as.character(sliding_window[,"P-value"])) < 0.05),]
    ...
    if (nrow(clusters) == 0)
      return(clusters)
    clusters <-
      clusters[order(clusters[,"Chromosome"],clusters[,"cSTART"]),]
    ...
    for (i in 1:length(names(clusters)))
    {
      ...
      if (limit == 0)
      {
        ...
        Seq <-
          data.frame(
            Chromosome = as.character(name), cSTART = as.numeric(cluster_start), cSTOP =
              as.numeric(cluster_end), minP = as.numeric(cluster_p_value)
          )
        #	print(Seq)
      ...
      else
      {
        ...
        for (j in 1:limit)
        {
          ...
          if ((start2 - start1) > 1000000)
          {
            ...
            cluster_p_value_next <-
              as.numeric(clusters[[i]][j,"P.value"])
