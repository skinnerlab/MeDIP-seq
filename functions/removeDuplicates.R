...
removeDuplicates <- function(mat1, mat2) {
  ...
  if (is.na(mat1[1])) return(cbind(list.index = 1, dmr.index = 1)[0, ])
  # If mat2 is NULL or NA, return mat1
  if (is.null(mat2)) return(mat1)
  if (is.na(mat2[1])) return(mat1)
  ...
  # This line prevents multiple row empty data.frames
  if (ncol(uniqueSet)==0) uniqueSet <- data.frame()
  return(uniqueSet)
}
