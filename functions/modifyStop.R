...
## This function makes sure the window stop site is actually on the chromosome. If it isn't, 
## the stop site is changed to the last position on the chromosome and the length is adjusted 
## accordingly.
...
modifyStop <- function(dmrList, refGenome, maxDMR) {
  ...
  if (nrow(dmrList) < maxDMR && nrow(dmrList) > 0) {
    for (rn in 1:nrow(dmrList)) {
      ...
      i$stop <- min(i$stop, eval(parse(text = paste("length(refGenome$", "\"", 
                                                    i$chr, "\"", ")", sep = ""))))
