...
This report summarizes the results from the `r projectName` project (`r comparisonNames[an]` analysis). It was generated on `r format(Sys.Date(), format="%B %d %Y")`.
...
names(chrLengths)<-paste(chrPrefix, names(chrLengths), sep="")
...
chrLengths<-seqlengths(eval(parse(text=referenceName)))
names(chrLengths)<-paste(chrPrefix, names(chrLengths), sep="")
plotChromosomes(siteTable=MTCmethList3p[[MTCpvc]], chrLengths=chrLengths, ymar=5)
```
